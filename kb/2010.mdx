...
description: "Simplify each query run through the environment configuration and execute conditional queries based on user input."
---
...
> 아래와 같이 API를 단축하여 사용하려면 환경변수(environmental variables) 설정이 필요합니다. `SELECT_QUERY_DEV_RO`로 설정하는 경우 `DEV_RO`로 활용합니다.
